...
<!-- Page-Title -->
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <center><h4 class="page-title">Cabinet</h4></center>
                    </div>
                </div>
...
<center>
		...
		<div class="card-box">
				<div class="blockMine" id="GHS">
					 <input type="hidden" id="OldBalance" value="<?=$BalanceGHS;?>" />
						...
						GHS</h4>
						<hr>
						Balance: <span id="Balance"><?=$BalanceGHS;?></span> GHS
						<hr>
						Price: <span id="Price"><?=sprintf("%.8f", $this->config->item('PriceCloudGHS'));?></span> BTC
						<hr>
						<?
						...
						?>
						Income per hour: ~<?=sprintf("%.8f", $SPedding * 3600);?> GHS
						<hr>
						
						Amount: <span id="Cash"><?=sprintf("%.8f", $this->config->item('PriceCloudGHS') * $BalanceGHS);?></span> BTC
						<hr>
		...
		<div class="card-box">
				...
				Advertising<br>
<!--
<a href="https://cloud.bitaltearning.com/Dashboard/Ticket"><img src="/tpl/dfhsfgsdf_en.jpg"></a>

<script src="//catcut.net/adv/14716"></script>
-->		
<a href="https://millhash.com/Welcome/Partner/1" target="_blank"><img src="https://millhash.com/aff/MH-200-EN.gif"></a>				
			...
			<div class="card-box">
				<div class="blockMine" id="<?=$Coin->cAbbr;?>">
					 <input type="hidden" id="OldBalance" value="<?=sprintf("%.15f", $Coin->bBalance);?>" />
						...
						<hr>
						Balance: <span id="Balance"><?=$Coin->bBalance;?></span> <?=$Coin->cAbbr;?>
						<hr>
						Price: <span id="Price"><?=$Coin->cPrice;?></span> BTC
						<hr>
						<?
						$SPedding = ((($this->config->item('PriceCloudGHS') * $BalanceGHS) / (60 * 60 * 24 * $this->config->item('ReturnDayFromDeposit'))) / $Coin->cPrice);
						?>
						Income per hour: ~<?=sprintf("%.8f", $SPedding * 3600);?> <?=$Coin->cAbbr;?>
						<hr>
						
						Amount: <span id="Cash"><?=sprintf("%.8f", $Coin->cPrice * $Coin->bBalance);?></span> BTC
						<hr>
						<input type="submit" 
