        ...
        success: function (data) {
        	if(data.error == 'no')
        	{
				if(data.action == 2)
				{
					...
					$('#ButtonCoin_'+data.Uid).removeClass('btn-danger').addClass('btn-success');
					$('#ButtonCoin_'+data.Uid).val('Отключить');

				}
				else if(data.action == 1)
				{
					$('#coin_'+data.Uid).removeClass('online').addClass('offline');
					$('#statusCoin_'+data.Uid).text('Offline');
					$('#ButtonCoin_'+data.Uid).removeClass('btn-success').addClass('btn-danger');
					$('#ButtonCoin_'+data.Uid).val('Включить');
				}
			}
...
function ChangeMinDeposit(num)
{
	$.ajax({
        type:"POST",
        dataType: 'JSON',
        data: jQuery("#Coins_" + num).serialize(),
        url:"/Flatpanel/Coins",
        success: function (data) {
        	if(data.status == 'ok')
        	{
				$('#cc_' + num).html('Сохранено');
			}
...
function UpdateBalanceUser(num)
{
	...
	$.ajax({
        ...
        dataType: 'JSON',
        data: jQuery("#Coins_" + num).serialize(),
        url:"/Flatpanel/UpdateBalanceUser",
        success: function (data) {
        	if(data.status == 'ok')
        	{
				$('#error_' + num).html('Сохранено');
			}
